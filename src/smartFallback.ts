...
interface FallbackResult {
    suggestion: string;
    source: 'gemini' | 'openai' | 'claude' | 'localai' | 'pattern' | 'template';
    confidence: number;
    latency_ms: number;
}
...
export class SmartFallback {
    ...
    async suggest(
        ...
        try {
            ...
            if (!cancellationToken.isCancellationRequested) {
                ...
                if (aiResult) {
                    // Determinar a fonte real (IA que respondeu)
                    const selectedProvider = (this.aiSelector as any)['selectedAI']?.provider;
                    const source = this.mapProviderToSource(selectedProvider);
    ...
    /**
     * Mapear provider interno para o tipo source do FallbackResult
     */
    private mapProviderToSource(provider: string | undefined): FallbackResult['source'] {
        switch (provider) {
            case 'gemini': return 'gemini';
            case 'openai': return 'openai';
            case 'claude': return 'claude';
            case 'localai':
            case 'ollama': return 'localai';
            default: return 'gemini'; // fallback padrão para compatibilidade
        }
    }
