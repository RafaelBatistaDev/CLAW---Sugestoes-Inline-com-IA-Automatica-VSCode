...
import { fetchWithTimeout, getAPIKey } from './utils';
...
export class AISelector {
    ...
    async callAI(
        ...
        try {
            ...
            // Sanitizar mensagem de erro para evitar vazamento de API keys
            const sanitizedMsg = errorMsg.replace(/(sk-[a-zA-Z0-9]{20,})/g, 'sk-***');
            this.logger.debug(`❌ ${this.selectedAI?.name} falhou: ${sanitizedMsg}`);
        ...
        // Tentar próxima IA (recursão controlada)
            ...
            const response = await fetchWithTimeout(
                'https://api.anthropic.com/v1/messages',
                {
                    ...
                    body: JSON.stringify({
                        ...
                        max_tokens: maxTokens,
                        messages: [
                            { role: 'user' as const, content: `${language} code: ${prompt}` }
                        ]
                    })
                },
                2000
