...
export class AgentManager {
    ...
    async getStatus(): Promise<string> {
        try {
            const result = execSync(
                `${this.pythonCommand} "${this.agentPythonPath}" status 2>&1`,
                { timeout: 3000, encoding: 'utf-8' as const }
            ) as string;
            return result;
        } catch {
    ...
    private resolveAgentPath(): string {
        ...
        // PathResolver.findAgentPy procura em localizações conhecidas
